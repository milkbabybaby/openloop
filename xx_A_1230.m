...
for kg=1:NN
          ...
          CB3=CB1*expm(A'*tf)*C*expm(A*(tf-x(kg)))*B*B';  % CB3=CB1*expm(A'*tf)*C*expm(A*(tf-kkk*ot))*B*B'+CB3;
       CC3=exp(V*(y(kg)-x(kg)));
CCC3=II*CC3;
...
% CCG=zeros(N,N);
%         for  kk=1:tf/ot  
%             CCG=expm(A*(tf-kk*ot))*B*B'*expm(A'*(tf-kk*ot))*ot+CCG;%没考虑初值为0
%         end
...
for kg=1:NN
            ...
            CD3=C*expm(A*tf)*CC4*expm(A*(y(kg)-x(kg)))*B*B';
        CC5=exp(V*(tf-x(kg)));
CCC5=II*CC5;
...
ot=0.02;tf=1;  
     ...
     CE3=zeros(N,N);
        for kg=1:NN
            ...
            CE3=[expm(A*(tf-x(kg)))*B*B'*expm(A'*(y(kg)-x(kg)))*CB1*expm(A'*tf)]+CE3;
...
%   F4=zeros(N,N);
%     for k=1:iiii
%         F4=A'^(k-1)*C*CE33*C*expm(A*x(kg))*B*B'*A'^(iiii-k)+F4;
%     end
% 
% F44=2*alpha_t(iiii+1)*F4+F44;
% end
...
ot=0.02;tf=1;
   ...
   CF3=zeros(N,N);
        for kg=1:NN
            ...
            CF3=[expm(A*tf)*CC4*expm(A*(y(kg)-x(kg)))*B*B'*expm(A'*(tf-x(kg)))]+CF3;
        end
        CF33=CF3/NN*0.5;
...
ot=0.02;tf=1;
   ...
   F666=zeros(N,N);
        for kg=1:NN
            ...
            CB66=C*expm(A*(tf-x(kg)))*B*B'*expm(A'*(y(kg)-x(kg)))*CB1;
      F66=zeros(N,N); 
for iiii=1:N-1
  F6=zeros(N,N);
    for k=1:iiii
...
%  id = cell2mat(id)syms s t
syms s t
z1= int(int(s^2*t^2, s, 0, t),t, 0,1)
