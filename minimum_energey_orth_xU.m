...
ii  
  ...
  hold on 
    ...
    plot(Cost3,'k-.')
    legend('x+u', 'x', 'u')
   % export_fig cost_alpha0.eps -painters -transparent
% % eval(subs(x,'t',tf))
% xxx=[];
...
legend([ez1,ez2(1)],'x0', ' x*',0)
for ii=1:N
set(ez2(ii),'color',[1 0 0])

end
 %export_fig x_alpha0.eps -painters -transparent
...
% legend('y1=t^2','y2=t^2-16')
delete(findall(findall(gcf,'Type','axe'),'Type','text'))
legend([ez1,ez2(1)],'u0', 'u*',0)
for ii=1:M
set(ez2(ii),'color',[1 0 0])

end
 %export_fig u_alpha0.eps -painters -transparent